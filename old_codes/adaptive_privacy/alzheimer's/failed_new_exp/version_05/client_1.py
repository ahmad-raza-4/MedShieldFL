...
def update_noise_multiplier(
    ...
    min_noise = base_noise_multiplier/2
    gamma = 200

    sample_rate = client_data_size / PARAMS["full_dataset_size"]

    # Compute a scalar Fisher value from the Fisher diagonal tensors (using the mean)
    fisher_values = []
    for f in fisher_diag:
        if isinstance(f, torch.Tensor):
            f = f.detach().cpu().numpy()
        fisher_values.append(np.mean(f))
    fisher_mean = np.mean(fisher_values)
    
    # Compute a "Fisher factor" that increases with epoch, fisher_mean, and client sample rate.
    fisher_factor = gamma * epoch * fisher_mean * sample_rate
    # Use an exponential ramp-up function.
    dynamic_noise_multiplier = min_noise + (base_noise_multiplier - min_noise) * (1 - math.exp(-fisher_factor))
    
    # Ensure the dynamic noise multiplier never falls below epsilon.
    dynamic_noise_multiplier = max(dynamic_noise_multiplier, epsilon)
    
    return dynamic_noise_multiplier
