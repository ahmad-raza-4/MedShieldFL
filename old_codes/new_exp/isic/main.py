...
# === Added CSV Initialization ===
csv_file = "results_fed_isic2019.csv"
...
for se in seeds:
    ...
    for epsilon, delta in product(epsilons, deltas):
        ...
        for client_idx, train_dl in enumerate(training_dls):
            ...
            fisher_diag = compute_fisher_information(Baseline(device=device), train_dl, device=device)  # Pass individual DataLoader
    ...
    # === Added CSV Writing for FedAvg without DP ===
    iteration += 1  # Increment iteration counter
    with open(csv_file, mode="a", newline="") as file:
        writer = csv.writer(file)
        writer.writerow([iteration, None, None, dynamic_noise_multiplier, mean_perf, se])
    # === End of CSV Writing for FedAvg without DP ===
    ...
    for e, d, dynamic_noise_multiplier in tqdm(edelta_list):
        ...
        # === Added CSV Writing for FedAvg with DP ===
        iteration += 1  # Increment iteration counter
        with open(csv_file, mode="a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow([iteration, e, d, dynamic_noise_multiplier, mean_perf, se])
        # === End of CSV Writing for FedAvg with DP ===

# After all iterations, optionally save the results to a DataFrame (already being saved incrementally)
# However, keeping it for completeness
results = pd.DataFrame.from_dict(results_all_reps)
results.to_csv("results_fed_isic2019_full.csv", index=False)
