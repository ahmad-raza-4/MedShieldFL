...
class FedIsic2019(Isic2019Raw):
    ...
    def __init__(
            ...
            """Cf class docstring"""
            sz = 224
            padding_size = 250  # New padding size before cropping

            if train:
                augmentations = albumentations.Compose(
                    [
                        albumentations.PadIfNeeded(min_height=padding_size, min_width=padding_size),
                        albumentations.RandomScale(0.07),
                        albumentations.Rotate(50),
                        albumentations.RandomBrightnessContrast(0.15, 0.1),
                        albumentations.Flip(p=0.5),
...
if __name__ == "__main__":
    ...
    for i in range(6):
        ...
        print(f"Train Dataset for Client {i}: {len(mydataset)}")
        # print("S0ize of image 0 ", mydataset[0][0].shape)
        mydataset = FedIsic2019(center=i, train=False, pooled=False)
        print(f"Test Dataset for Client {i}: {len(mydataset)}")
        # print("Size of image 0 ", mydataset[0][0].shape)
