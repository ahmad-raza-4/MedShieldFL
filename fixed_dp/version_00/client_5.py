...
def test(net, testloader):
    ...
    string_labels=f"Unique values in labels_array: {np.unique(labels_array)}"
    string_scores=f"Unique values in scores_array: {np.unique(scores_array)}"
    save_str_to_file(string_labels, client_name)
    save_str_to_file(string_scores, client_name)
...
class FedViTDPClient5(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        print(f"Epsilon for {client_name}= {epsilon:.2f}")
        return (
...
fl.client.start_client(
    ...
    client = FedViTDPClient5(model=model, trainloader=trainload, testloader=testloader).to_client()
)
