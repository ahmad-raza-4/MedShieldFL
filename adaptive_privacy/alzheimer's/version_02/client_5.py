...
#     base_noise_multiplier, fisher_diag, client_data_size, epoch, max_epochs, epsilon
# ):
#     # Convert Fisher values to numpy if they are tensors
#     fisher_scaling_factor = [f.detach().cpu().numpy() if isinstance(f, torch.Tensor) else f for f in fisher_diag]
...
#     # Aggregate using mean (you can also use sum or weighted average)
#     noise_multiplier = np.mean(noise_multipliers)

#     # Optional: Apply ε scaling (uncomment if needed)
#     # noise_multiplier *= (1 / epsilon)  

#     # Optional: Apply convergence factor (uncomment if needed)
