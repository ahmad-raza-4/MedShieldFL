...
class FedViTDPClient5(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        train_epsilon = self.privacy_engine.get_epsilon(delta)

        string = f"Train Dataset Size: {len(self.trainloader)} Sample rate: {sample_rate}, Epsilon: {train_epsilon:.4f}, Loss: {avg_train_loss:.4f}"
...
fl.client.start_client(
    server_address="127.0.0.1:8025",
    client = FedViTDPClient5(model=model, trainloader=trainload, testloader=testloader, sample_rate=sample_rate).to_client()
)
